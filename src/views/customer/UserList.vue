<template>
  <div class="userList">
          ...
          <div class="header">
            ...
            <div class="header_bottom">
              <div class="search_left">
                <a-select @change="handleChange1" class="select_identity" size="large" placeholder="身份">
                  <a-select-option
                    :value="item.id"
                    v-for="item in identityList"
                    :key="item.id"
                  >{{item.identity}}</a-select-option>
                </a-select>

                <a-select @change="handleChange2" class="select_logonmode" size="large" placeholder="注册方式">
                  <a-select-option
                    :value="item.id"
                    v-for="item in designStyleList"
                    :key="item.id"
                  >{{item.designStyle}}</a-select-option>
                </a-select>

                <a-select @change="handleChange3" class="select_attribution" size="large" placeholder="归属">
                  <a-select-option
                    :value="item.id"
                    v-for="item in attributionList"
                    :key="item.id"
                  >{{item.attribution}}</a-select-option>
                </a-select>
              </div>
    ...
    <a-modal
      ...
      :mask="true"
      width="430px"
      :bodyStyle="{'padding-bottom':'15px','margin-left':'-15px','padding-left':'0'}"
    >
    ...
    >
      <a-form-model v-bind="privilegeLayout" :model="privilegeForm">
        ...
        >
          <a-input
            v-model="privilegeForm.openPrice"
            style="width: 60%;margin-left:10px"
            type="text"
            placeholder="请输入价格"
          />
...
export default {
  ...
  data() {
    return {
      ...
      //注册方式
      designStyleList: [
        { id: 1, designStyle: "app" },
        { id: 2, designStyle: "ios" },
        { id: 3, designStyle: "微信" }
      ],
      //归属
      attributionList: [
  ...
  methods: {
    ...
    chooseTime(date, dateString) {
      ...
      this.rechargeForm.date = dateString;
    },
    //充值框点确定后执行
    handleOk() {
      this.visible = false;
    ...
    selectRegion(v) {
      console.log(v);
      this.privilegeForm.region = v;
    },

    //选择到期日期
    chooseDate(date, dateString) {
      // console.log(date, dateString);
      this.privilegeForm.date = dateString;
    },
